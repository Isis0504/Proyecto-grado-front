...
`;
  ...
  if (usuario.rol === "administrador") {
    ...
    nombreInput.addEventListener("input", () => {
      ...
      temporizador = setTimeout(() => cargarPagos(), 400); // espera mientras se escribe
    });
...
async function cargarPagos() {
  ...
  let query = supabase.from("pagos").select(`
    id, monto, fecha_pago, descripcion, soporte_url, estado,
  ...
  if (usuario.rol !== "administrador") {
    ...
    // no existe búsqueda "like" entre tablas relacionadas, así que se filtra luego en memoria
